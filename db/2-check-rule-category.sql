-- Table: camdecmpsaux.check_rule_category

-- DROP TABLE camdecmpsaux.check_rule_category;

CREATE TABLE camdecmpsaux.check_rule_category
(
    check_rule_category_cd character varying NOT NULL,
    check_rule_category_desc character varying NOT NULL,
    check_rule_category_process_cd character varying NOT NULL,
    check_rule_category_order integer NOT NULL,
    flex_source_id uuid NOT NULL,
    CONSTRAINT pk_check_rule_category PRIMARY KEY (check_rule_category_cd),
    CONSTRAINT uq_check_rule_flex_source UNIQUE (flex_source_id),
    CONSTRAINT fk_check_rule_category_process FOREIGN KEY (check_rule_category_process_cd)
        REFERENCES camdecmpsaux.check_rule_process (check_rule_process_cd) MATCH SIMPLE
);

COMMENT ON TABLE camdecmpsaux.check_rule_category
    IS 'Lookup table of check rule categories.';

COMMENT ON COLUMN camdecmpsaux.check_rule_category.check_rule_category_cd
    IS 'Code used to identify check rule category.';

COMMENT ON COLUMN camdecmpsaux.check_rule_category.check_rule_category_desc
    IS 'Description of check rule category.';

COMMENT ON COLUMN camdecmpsaux.check_rule_category.check_rule_category_process_cd
    IS 'The check rule process the check rule category is associated with.';

COMMENT ON COLUMN camdecmpsaux.check_rule_category.check_rule_category_order
    IS 'The priority order in which check rule categories are processed.';

COMMENT ON COLUMN camdecmpsaux.check_rule_category.flex_source_id
    IS 'Flex Source system type guid required by Code Effects rule editor.';

-- Index: idx_check_rule_category_process

-- DROP INDEX camdecmpsaux.idx_check_rule_category_process;

CREATE INDEX idx_check_rule_category_process
    ON camdecmpsaux.check_rule_category USING btree
    (check_rule_category_process_cd ASC NULLS LAST);

-- https://www.uuidgenerator.net/version4

-- MONITOR PLAN --
insert into camdecmpsaux.check_rule_category(check_rule_category_cd, check_rule_category_desc, check_rule_category_process_cd, check_rule_category_order, flex_source_id)
